...
switch($_GET["tipe"]){
	...
	case "1": 
	$c = mysqli_query($con, "SELECT count(nama) as jum, id_pasien from history_kasir where jenis='Treatment' group by id_pasien having jum>='$_GET[val]'"); 
	...
	case "3": 
	$c = mysqli_query($con, "SELECT * from pembayaran where total>='$_GET[val]' group by id_pasien"); 
	while($cx = mysqli_fetch_assoc($c)){
		$xz .= "'".$cx['id_pasien']."',";
	}
	$xz = substr($xz,0,strlen($xz) - 1);
