<?php
						...
						while (list($key, $value) = each($gdg)) 
							{
								...
								$trg_stok	= mysqli_query($con, 'Create Trigger barang_keluar After Insert On barang_keluar
								For Each Row Begin
								Update obat_stok Set 
								jumlah 			= jumlah - New.jumlah
								Where id_brg	= New.id_brg;
								End');							
								$brg_keluar	= mysqli_query($con, "Insert Into barang_keluar(no_fbk,id_bk,jumlah,jam_bk,tgl_bk)
											  VALUES('$fbk','$gdg','$jum','$jam','$tgl')");  
							}
						}		
		catat($con, $_SESSION['namauser'], 'Data Barang Keluar Baru'.' ('.$fbk.')');
		if($brg_keluar){
		?>
	...
	if ($module	=='barang_keluar' AND $act=='del'){
		$id			= $_GET['id'];
		$delete		= mysqli_query($con, "Delete From barang_keluar Where no_fbk='$id'");
		$trg_stok	= mysqli_query($con, "Create Trigger del_bk After Delete On barang_keluar
		For Each Row Begin
		Update barang_stok Set 
		jumlah 			= jumlah + OLD.jumlah
		Where id_obat	= OLD.id_obat;
		End");							
		catat($con, $_SESSION['namauser'], 'Hapus Data Barang Keluar'.' ('.$id.')');
