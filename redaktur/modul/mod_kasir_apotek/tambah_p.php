...
$id_dr = $_SESSION['id_dr'];
	...
	$q1 = mysql_query("SELECT *FROM history_kasir WHERE nama='$nama' AND no_faktur='$no_faktur'");
	...
	if($jum>0){ 
		...
		$sisastok = $stok[jumlah] - $jumlah;
		mysql_query("UPDATE produk SET jumlah = $sisastok WHERE nama_p = '$nama'");
		mysql_query("INSERT INTO history_kasir(nama,jumlah,harga,id_kk,harga_beli,kode,jenis,id_pasien,status,no_urut,tanggal,penginput,no_faktur,diskon,sub_total,ket,id_dr,kategori) VALUES('$nama','$jumlah','$harga','$id_kk','$harga_b','$kode','Produk','$id','Belum Lunas','$no','$tgl','Apotek','$no_faktur','$diskon','$sub_total','$ket','$id_dr',0)");
	}
