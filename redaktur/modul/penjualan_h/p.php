...
$cb = mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM daftar_klinik WHERE id_kk = '$_GET[klinik]'"));
?>
...
<hr/>
        ...
        $q1 = mysqli_query($con,"SELECT history_kasir.*, SUM(harga*jumlah) AS subtotal, SUM(jumlah) AS jml FROM history_kasir WHERE $tgl AND id_kk='$_GET[klinik]'  GROUP BY nama");
          ...
          <tr>
              ...
              while ($br2 = mysqli_fetch_array($q2)) {
                ...
                $his = mysqli_query($con,"SELECT * FROM history_kasir WHERE id = '$br2[history]' AND id_kk = '$_GET[klinik]'");
                if(mysqli_num_rows($his) > 0){

                $hist = mysqli_fetch_assoc($his);
                $prod = mysqli_fetch_assoc(mysqli_query($con,"SELECT nama_p FROM produk WHERE id_kk = '$_GET[klinik]' AND kode_barang = '$br2[replaces]'"));
                $prod2 = mysqli_fetch_assoc(mysqli_query($con,"SELECT nama_p FROM produk WHERE id_kk = '$_GET[klinik]' AND kode_barang = '$hist[nama]'"));
          ...
          <tr>
              ...
              $q3 = mysqli_query($con,"SELECT * FROM bonus WHERE $tgl3 AND klinik = '$_GET[klinik]'");
        
              while ($br3 = mysqli_fetch_array($q3)) {
                $prod2 = mysqli_fetch_assoc(mysqli_query($con,"SELECT nama_p FROM produk WHERE id_kk = '$_GET[klinik]' AND kode_barang = '$br3[produk]'"));
