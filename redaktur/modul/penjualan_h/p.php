...
$cb = mysql_fetch_assoc(mysql_query("SELECT * FROM daftar_klinik WHERE id_kk = '$_GET[klinik]'"));
?>
...
<hr/>
        ...
        $q1 = mysql_query("SELECT history_kasir.*, SUM(harga*jumlah) AS subtotal, SUM(jumlah) AS jml FROM history_kasir WHERE $tgl AND id_kk='$_GET[klinik]'  GROUP BY nama");
          ...
          <tr>
              ...
              while ($br2 = mysql_fetch_array($q2)) {
                ...
                $his = mysql_query("SELECT * FROM history_kasir WHERE id = '$br2[history]' AND id_kk = '$_GET[klinik]'");
                if(mysql_num_rows($his) > 0){

                $hist = mysql_fetch_assoc($his);
                $prod = mysql_fetch_assoc(mysql_query("SELECT nama_p FROM produk WHERE id_kk = '$_GET[klinik]' AND kode_barang = '$br2[replaces]'"));
                $prod2 = mysql_fetch_assoc(mysql_query("SELECT nama_p FROM produk WHERE id_kk = '$_GET[klinik]' AND kode_barang = '$hist[nama]'"));
          ...
          <tr>
              ...
              $q3 = mysql_query("SELECT * FROM bonus WHERE $tgl3 AND klinik = '$_GET[klinik]'");
        
              while ($br3 = mysql_fetch_array($q3)) {
                $prod2 = mysql_fetch_assoc(mysql_query("SELECT nama_p FROM produk WHERE id_kk = '$_GET[klinik]' AND kode_barang = '$br3[produk]'"));
