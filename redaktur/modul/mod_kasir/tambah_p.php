...
if(mysql_num_rows($c) > 0){
    ...
    $ant = mysql_fetch_assoc(mysql_query("SELECT MAX(no_urut) ree FROM antrian_pasien WHERE tanggal_pendaftaran = '$now' AND jenis_layanan = 'poliklinik'"));
    if(is_null($ant["ree"])){
        $antrian = 1;
    } else {
        $antrian = $ant["ree"] + 1;
    ...
    mysql_query("INSERT INTO antrian_pasien (suhu,tensi,id_kasir,no_urut,no_faktur,id_pasien,tanggal_pendaftaran,status,kunjungan_ke,jenis_layanan,jenis_pasien,id_dr,asuransi,poliklinik,bb,tb,keluhan) VALUES ('$_POST[suhu]','$_POST[tensi]',$_POST[id_kasir],$antrian,'$_POST[nofak]','$_POST[id_pasien]','$now',NULL,$kunjungan,'poliklinik','$_POST[pas]',$dr,$asuransi,$poli,$_POST[bb],$_POST[tb],'$_POST[sakit]')");
