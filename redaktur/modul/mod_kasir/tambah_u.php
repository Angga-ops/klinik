...
if(mysql_num_rows($c) > 0){
    ...
    $ant = mysql_fetch_assoc(mysql_query("SELECT MAX(no_urut) ree FROM antrian_pasien WHERE tanggal_pendaftaran = '$now' AND jenis_layanan = 'igd'"));
    if(is_null($ant["ree"])){
        $antrian = 1;
    } else {
        $antrian = $ant["ree"] + 1;
